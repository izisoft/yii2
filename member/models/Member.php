...
class Member extends ActiveRecord implements IdentityInterface
{
    ...
    private static $_typeId = TYPE_ID_MEMBER;
    
    public static function setTypeId($type_id)
    {
        Member::$_typeId = $type_id;
    }
    ...
    {    	
    	...
    	return $u->andWhere(['type_id' => Member::$_typeId, 'is_active' => 1,  'status' => self::STATUS_ACTIVE]+($username === ROOT_USER ? [] : ['sid'=>__SID__]))->one();
    ...
    public function findByUsername2($username)
    {
    	...
    	$u->andWhere(['sid'=>__SID__, 'type_id' => Member::$_typeId]);
...
//             isset(Yii::$site['settings']['customers'][Member::$_typeId]['code']) ?
//             Yii::$site['settings']['customers'][Member::$_typeId]['code'] : []));
...
//             ]);
    ...
    public static function registerFromGoogle($member = []){
    	$user = new Member();
    	$password = randString(6);
...
//     			Yii::$site['settings']['customers'][Member::$_typeId]['code'] : []));
    ...
    public static function updatePassword($item,$o = []){
    	...
    	if(!empty($item) && !empty($shop)){
    		...
    		$replace = array(
    				isset(Yii::$app->cfg->app['logo']['image']) ? '<img src="' . Yii::$app->cfg->app['logo']['image']  .'" height="60"/>': '',
    				$shop['domain'],
...
//     		$text = Yii::$app->getTextRespon(array('code'=>'RP_SENDPASS', 'show'=>false));
    		$text = Yii::$app->frontend->getTextRespon(array('code'=>'RP_SENDPASS', 'show'=>false));
    		//view($text); exit;
