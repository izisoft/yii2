...
class Model extends \yii\db\ActiveRecord
{
	...
	public static function getListCurrency($cache=false){
		$c = Yii::$app->db->getConfigs(self::$key,false,__SID__,$cache);
	...
	public function setDefaultCurrency($id, $cache=true){
		$c = Yii::$app->db->getConfigs($this->key,false,__SID__,false);
	...
	public static function getCurrencies(){
		$c = Yii::$app->db->getConfigs(self::$key,false,__SID__,false);
...
// 		$sql .= $time !== false ? " and DAYOFYEAR(a.from_date)=".date('z',$time) . " and YEAR(a.from_date)=" . date('Y',$time) : '';
		$sql .= $time !== false ? " and UNIX_TIMESTAMP(a.from_date)<$time" : '';
	...
	public function getLastestExchangeRate($params)
	{
		...
		$query = Model::find()
		->select($select)
		->from($this->tableExchangeRate())->where([
			'from_currency' => $from,
			'to_currency' => $to,
		]);
		
		if(isset($params['before_date'])){
			$query->andWhere(['<', 'updated_at', strtotime($params['before_date'])]);
		}
		
		$r = $query->orderBy(['updated_at'=>SORT_DESC])->asArray()->one();
		
		/*
		if(!empty($r) && isset($params['reverse']) && $params['reverse'] === true){
    	    
    	    $old_amount = $r['amount'];
    	    $old_value  = $r['value'];
    	    
    	    if($old_amount / $r['value'] < 1/10000){
    	        $r['amount'] *= 10000;
    	    }
    	    ...
    	    $r['value']    = $old_amount / $r['value'] * $r['amount'] ;
    	    $r['sell']     = $old_amount / $r['sell'] * $r['amount'] ;
    	    
    	    $r['increase'] = ($old_amount / $old_value) * $r['increase'];
