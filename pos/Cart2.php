...
class Cart2 extends \yii\base\Component
{
    ...
    {
        $this->session->set($this->_key_store_version, $this->_version);
    ...
    public function add($params)
    {
        ...
        $prices = Yii::$app->product->getPrices($item['item_id']);
        
        $item['price']     =   $prices->sale_price;
        $item['prices'] = $prices;
        
        $item['customer_id']    =   isset($params['customer_id']) ? $params['customer_id'] : 0;
        $item['seller_id']    =   isset($params['seller_id']) ? $params['seller_id'] : 0;
        $item['created_at'] = $item['last_modify'] = time();
        $item['sid'] = __SID__;
        
        $item['root_item_id'] = isset($params['root_item_id']) ? $params['root_item_id'] : $params['item_id'];
...
//         view($l)
        ...
        if(!empty($l)){
            foreach($l as $item_id => $v){
                ...
                if(!empty($prices)){
                    $l[$item_id]['price'] = $prices->sale_price;
                }
            }
        }
...
//             return $session->get($this->_session_name);
        ...
        if(!empty($cart)){
            ...
            foreach ($cart as $item_id => $item){
                ...
                $root_item_id = isset($item['root_item_id']) ?$item['root_item_id'] : $item_id;
                
                $prices = Yii::$app->product->getPrices($item_id);
                
                $i = Yii::$app->product->model->getItem($item_id);
                $i2 = Yii::$app->product->model->getItem($root_item_id);
                if(!empty($i)){

                    $total_price    +=  $item['quantity'] * $prices->sale_price;
                    
                    $icon = Yii::$app->product->getAttrValue($item_id, 'icon');
                    
                    if($icon == ""){
                        $icon = Yii::$app->product->getAttrValue($root_item_id, 'icon');
                    }
                    $list_images   =  Yii::$app->product->getAttrValue($item_id, 'images');
                    if($list_images == ""){
                        $list_images   =  Yii::$app->product->getAttrValue($root_item_id, 'images');
                    }
                    ...
                    $itemd = [
                        ...
                        'id'            =>  $item_id,
                        'sku'          =>  $i->sku,
                        'name'         =>  ($name = Yii::$app->product->getAttrValue($root_item_id, 'name')),
                        'type_id'       =>  $i2->type_id,
                        'title'         =>  $name,
                        'url'           =>  Yii::$app->product->getAttrValue($root_item_id, 'url'),
                        'url_link'      =>  Yii::$app->product->getAttrValue($root_item_id, 'url_link'),
                        'price'        =>  $prices->sale_price,
                        'compare_price'        =>  $prices->compare_price,
                        'prices'        => $prices,
//                         'price2'        =>  $i['price2'],
//                         'price3'        =>  $i['price3'],
                        'quantity'      =>  $item['quantity'],
                        'amount'        =>  $item['quantity'] * $prices->sale_price,
                        'icon'          =>  $icon,
                        'currency'      =>  max(1, Yii::$app->product->getPrice($item_id, 'currency')),
                    ...
                    //if($item['seller_id'] > 0){
                        ...
                        $seller[$item['seller_id']]['total_price'] = (isset($seller[$item['seller_id']]['total_price']) ? $seller[$item['seller_id']]['total_price'] : 0) + ($item['quantity'] * $prices->sale_price);
                    //}
...
//             }
     ...
     */
    public function getSessionItems($item_id = 0)
    { 
