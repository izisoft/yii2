...
',
            ...
            '{{%ORDER_PRODUCTS_LIST}}'	=>	$this->renderOrderProducts($order['code']),
...
<td colspan="12" style="border: 1px solid #ddd;padding: 8px;line-height: 1.42857143;vertical-align: middle;font-size: 12px;text-align:left"><b>Ghi chú: </b> '.uh($cart['note']).'</td>
    ...
    public function renderOrderProducts($order_code)
    {
        $html = '';
...
<th colspan="2" style="border: 1px solid #ddd;padding: 8px;line-height: 1.42857143;vertical-align: middle;font-size: 12px;text-align:center;background-color:#dedede;">Đơn giá</th>
            ...
            foreach ($order['data'] as $item_id => $item){
                ...
                if(!empty($item_info)){
                    ...
                    {
                        $item_info['price2'] = $item['price2'];
                    }
                    
                    $html .= '<tr>
<td colspan="9" style="border: 1px solid #ddd;padding: 8px;line-height: 1.42857143;vertical-align: middle;font-size: 12px;"><a target="_blank" href="'.getAbsoluteUrl($item_info['url_link']).'">'.uh($item_info['title']).'</a></td>
...
<td colspan="2" style="border: 1px solid #ddd;padding: 8px;line-height: 1.42857143;vertical-align: middle;font-size: 12px;text-align:right"><b style="color:red">'.Yii::$app->frontend->showPrice($item_info['price2'],$order['currency']).'</b></td>
</tr>';
                }
            }
