...
use izi\models\Store;
use izi\models\StoreGroup;
use izi\models\StoreWebsite;
use Yii;
use izi\models\DomainPointer;
use izi\user\models\User;
use izi\models\UserToShop;
use izi\models\TempToShop;
...
class Shop extends \yii\base\Component
{
	...
	{
		$this->initStoreWebsite();
	}

	public function initStoreWebsite()
	{
		$web = StoreWebsite::findOne(['sid' => __SID__, 'is_default'=>1]);
		if(empty($web)){
			$web = new StoreWebsite();
			$web->sid = __SID__;
			$web->code = 'base';
			$web->name = 'Main Website';
			$web->is_default = 1;
			$web->save();
		}
		// 
		$group = StoreGroup::findOne(['website_id' => $web->website_id]);
		
		if(empty($group)){
			$group = new StoreGroup();
			$group->website_id = $web->website_id;
			$group->name = 'Main website store';
			$group->code = 'main_website_store';			
			$group->save();
		}
		// store
		$store = Store::findOne(['website_id' => $web->website_id, 'group_id' => $group->group_id]);
		if(empty($store)){
		   $store = new Store();
		   $store->website_id = $web->website_id;
		   $store->group_id = $group->group_id;
		   $store->name = 'Defaulte store view';
		   $store->code = 'default';	
		   $store->is_active = 1;		
		   $store->save();

		   // update group
		   $group->default_store_id = $store->store_id;
		   $group->save();
	   }
	}
	...
	public function updateDomainPointer($domain, $shop_id) {
	    
	    if(!empty($domain)){
	        $domains = [];
	        
	        // Explode all domain to array & add www version
	        ...
	        foreach ($domain as $d){
	            ...
	            foreach ($dx as $dz){
	                $dz = trim($dz);
	                if(validate_domain($dz)){
    	                
	                    $d1 = preg_replace('/^www\./i', '', $dz);
    	                
    	                $d2 = "www.$d1";
    	                
    	                if(!in_array($d1, $domains)) $domains[] = $d1;
    	                if(!in_array($d2, $domains)) $domains[] = $d2;
    	            }
	            }
	        }
	        
	        // Remove unselected domain
	        DomainPointer::deleteAll(['and' , ['sid' => $shop_id], ['not in', 'domain', $domains]]);
	        
	        if(!empty($domains)){
	            
	            // Remove default status
	            DomainPointer::updateAll(['is_default' => 2], ['sid' => $shop_id]);
	            
	            foreach ($domains as $k => $domain){
	        
	                $d1 = ['sid' => $shop_id, 'domain' => $domain];	                    
	                ...
	                $d = DomainPointer::findOne(['sid' => $shop_id, 'domain' => $domain]);
	...
	/**
	 * Register Primary Email && add to admin group
	 * 
	 */
	
	public function registerPrimaryEmail($email, $shop_id)
	{
	    if(validateEmail($email) && $shop_id>0){
	        // Validate user by email
	        $user = User::find()->where(['sid' => $shop_id, 'email' => $email])->one();
	        ...
	        if(!empty($user)){
	            ...
	            Yii::$app->authManager->assign(Yii::$app->authManager->createRole(ADMIN_USER), $user->id);
	            
// 	            Yii::$app->user->resetPassword($user, $shop_id);
	            ...
	            if($user->save()){
	                ...
	                // Đăng ký phân quyền vào nhóm Admin
	                Yii::$app->authManager->assign(Yii::$app->authManager->createRole(ADMIN_USER), $user->id);
	        ...
	        // Cập nhât bảng User - Shop
	        $s = UserToShop::findOne(['user_id' => $user->id, 'sid' => $shop_id]);
	...
	public function registerTemplate($temps, $shop_id, $remove_other = false){
	    ...
	    if(!empty($temps)){
	        ...
	        //////////////////////////////////////
	        if($remove_other){
    	        TempToShop::deleteAll(['and',
    	            ['sid' => $shop_id],
    	            ['not in', 'temp_id', $temps]
    	        ]);
	        }
	        
	        //////////////////////////////////////
	        
	        TempToShop::updateAll(['state' => 2], ['sid' => $shop_id, 'state' => 1]);
	        
	        foreach ($temps as $k=>$temp_id){
	            $t = TempToShop::findOne(['sid' => $shop_id, 'temp_id' => $temp_id]);
	...
	public function updateContact($data, $shop_id)
	{
	    $conditions = ['sid' => $shop_id, 'code' => 'CONTACTS'];
	    
	    return SiteConfigs::updateData($data, $conditions);
