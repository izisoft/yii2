...
class Menu extends \izi\db\ActiveRecord
{
    ...
    public function getItem($id){
        ...
        ->where(['a.sid'=>__SID__, 'id'=>$id , 'lang' => __LANG__])
        ;
    ...
    public function getSmenuItems($param = []){
        $query = Smenu::find()->where(['sid' => __SID__]);
        
        return $query->all();
    }
    
    
    public function getSmenuItem($menu_id){
        $query = Smenu::find()->where(['sid' => __SID__, 'id' => $menu_id]);        
        $item = $query->asArray()->one();
        
        if(!empty($item)){
            $json_data = json_decode($item['json_data'],1);
