...
class Member extends Component
{
     ...
     */
    public $loginUrl = ['collaborator/login'];
    /**
    ...
    {    	
        parent::init();
		$this->loginUrl = ['collaborator/login'];		
    ...
    public function getModel(){
        if ($this->_model === null) {
            $this->_model = Yii::createObject(['class'=>$this->identityClass]);
        }
        return $this->_model;
    }
    ...
    public function getLevel(){
        $u = $this->getIdentity();
        return isset($u->level) ? $u->level : -1;
    }
    
    public function getGroupId(){
        $u = $this->getIdentity();
        return isset($u->group_id) ? $u->group_id : 0;
    }
    ...
    public function parsePhoneLogin($phone_number)
    {
        return parsePhoneNumber($phone_number);
    }
    
    public function getMember($rule , $params = []){
        $type_id = isset($params['type_id']) ? $params['type_id'] : TYPE_ID_CUSTOMER;
    	$query = (new \yii\db\Query())->from($this->getModel()->tableName())
    	->where(['sid'=>__SID__,'type_id'=>$type_id])
    	;
    	if(validateEmail($rule)){
    		$query->andWhere(['email'=>$rule]);
    	}elseif($rule != "" && in_array(substr($rule,0,1), [0,'+'])){
    	    $query->andWhere(['phone_login'=>$this->parsePhoneLogin($rule)]);
    	}else{
    		return;
    	}
    ...
    { 
        $info = parseMemberInfo($info);
      
        
        $rule = isset($info['email']) && validateEmail($info['email']) ? $info['email'] : (isset($info['phone']) ? $this->parsePhoneLogin($info['phone']) : '');
        
        $member = $this->getMember($rule);
        ...
        if(isset($info['phone']) && strlen(trim($info['phone'])) > 7){
            $info['phone_login'] = parsePhoneNumber($info['phone']);
        }
        
        if(!isset($info['password_hash'])){
            $password = randString(6);
            $info['password_hash'] = Yii::$app->security->generatePasswordHash($password);
            $info['auth_key'] = Yii::$app->security->generateRandomString();
        }
        $info['created_at'] = $info['updated_at'] = time();
        
        $info['status'] = isset($info['status']) ? $info['status'] : \izi\collaborator\models\Member::STATUS_ACTIVE;
        $info['type_id'] = isset($info['type_id']) ? $info['type_id'] : TYPE_ID_CUSTOMER;
        
        $info['sid'] = __SID__;
        
        $member = new \izi\collaborator\models\Member();
    ...
    public function genCode($id = 0, $i = 0)
    {
        $full_char = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890';
        
        $regex = isset(Yii::$app->cfg->app['settings']['customers'][TYPE_ID_MEMBER]) ? Yii::$app->cfg->app['settings']['customers'][TYPE_ID_MEMBER] : [];
        
        if(!empty($regex) && isset($regex['fields']['code']) && $regex['fields']['code'] == 'on'){

            $regex = $regex['code'];
            
            // /[a-zA-Z0-9]+/ : All char
            
            $pattern = $regex_string = $regex['code_regex'];
            
            if(@preg_match($pattern, $full_char, $match)){
                if(!empty($match)){
                    $regex_string = $match[0];
                }
            }
...
//             if(isset($regex['auto_code']) && $regex['auto_code'] == 'on'){
            ...
            if(!$sort_asc){
                // Nếu ko cần đánh số theo thứ tự tăng dần
                $code = randString($code_length,$regex_string);
                
                
            }else{
                // Đánh số thứ tự tăng dần
//                 $max_id = (new \yii\db\Query())->from($this->getModel()->tableName())->where(['sid'=>__SID__])->max('id');
                ...
                $sql .= ") from {$this->getModel()->tableName()} where sid=".__SID__;
                
                $max_id = Yii::$app->db->createCommand($sql)->queryScalar();
                
                $max_id = is_numeric($max_id) ? ++$max_id : 1;
                
                $code = danhso($max_id, $code_length);
            ...
            if((new \yii\db\Query())->from($this->getModel()->tableName())->where(['code'=>$code,'sid'=>__SID__])->count(1) >0){
                if($i > substr('999999999999999999999', 0,$code_length)){
                    return false;
                }
                return $this->genCode($id, ++$i);
            }
            return $code;
