...
class UrlManager extends \yii\web\UrlManager
{
    ...
    public function init()
    {
        ...
        //         $defaultTheme = 'welcome';
        
        Yii::$app->view->theme = new \izi\web\Theme([
            //            'pathMap'   =>  [
            //                '@app/views'     =>  "@app/themes/{$defaultTheme}",
            
            //                //@app/modules/blog/views/comment/index.php into @app/themes/basic/modules/blog/views/comment/index.php
            //                '@app/modules'   =>  "@app/themes/{$defaultTheme}/modules",
            
            //                //@app/widgets/currency/views/index.php into @app/themes/basic/widgets/currency/views/index.php
            //                '@app/widgets'   =>  "@app/themes/{$defaultTheme}/widgets",
    ...
    public function parseFrontendRequest($request)
    {
        ...
        if(!empty($this->_router)){
                ...
                switch ($k) {
                    case 'controller':
                        ...
                        if(!empty($this->_slug)){
                            ...
                            if($this->suffix == '' && isset($this->_router['controller']) && strrpos($this->_router['controller'], '.html')){
                                $this->_router['controller'] = str_replace(['.html', '.htm'], '', $this->_router['controller']);
                                
                                Yii::$app->response->redirect('/' . $this->_router['controller'], 301);
                                Yii::$app->end();
                            }
                        }
            ...
            if(!$is_validate_url && isset($this->_router['action'])
                && !in_array($this->_router['controller'], $catchAllController)
